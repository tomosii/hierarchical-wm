...
def set_exp_name(args):
    ...
    exp_name += "_b{}_s{}_c{}".format(
        args.belief_size, args.state_size, args.num_layers
    )
...
def main():
    ...
    wandb.init(project="hrssm", name=exp_name)
    ...
    # test data
    pre_test_full_data_list = next(iter(test_loader))
    ...
    while b_idx <= args.max_iters:
        ...
        for train_obs_list in train_loader:
            ...
            if b_idx % 100 == 0:
                ...
                wandb.log({"train/total_loss": loss}, step=b_idx)
            ...
            if b_idx % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    #     global_step=b_idx,
                    # )
                    wandb.log({"valid/total_loss": loss}, step=b_idx)
                    wandb.log({"valid/rec_image": wandb.Image(output_img)}, step=b_idx)
                    wandb.log(
                        {"valid/mask_image": wandb.Image(output_mask)}, step=b_idx
                    )
                    ...
                    # writer.add_image(
                    #     "valid/full_gen_image", output_img.transpose([2, 0, 1]), b_idx
                    # )
                    wandb.log(
                        {"valid/full_gen_image": wandb.Image(output_img)}, step=b_idx
                    )
                    ...
                    # log
                    output_img = plot_gen(
                        post_test_init_data_list, post_test_gen_data_list
                    )
                    # writer.add_image(
                    #     "valid/jumpy_gen_image", output_img.transpose([2, 0, 1]), b_idx
                    # )
                    wandb.log(
                        {"valid/jumpy_gen_image": wandb.Image(output_img)}, step=b_idx
                    )
