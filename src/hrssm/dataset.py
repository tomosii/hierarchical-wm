...
class MazeDataset(Dataset):
    def __init__(self, length, partition="train", path="3dmaze_32.npy"):
        ...
        if self.partition == "train":
            # (B, T, C, H, W)
            self.state = dataset[:num_seqs].transpose(0, 1, 4, 2, 3) / 1.0
        else:
            self.state = dataset[num_seqs:].transpose(0, 1, 4, 2, 3) / 1.0
...
def maze_dataloader(
    seq_size, init_size, batch_size, test_size=16, data_path="3dmaze_32.npy"
):
    train_loader = MazeDataset(
        length=seq_size + init_size * 2, partition="train", path=data_path
    )
    test_loader = MazeDataset(
        length=seq_size + init_size * 2, partition="test", path=data_path
    )
...
def bounce_dataloader(
    seq_size, init_size, batch_size, test_size=16, data_path="bouncing_ball_black.npy"
):
    train_loader = BounceDataset(
        length=seq_size + init_size * 2, partition="train", path=data_path
    )
    test_loader = BounceDataset(
        length=seq_size + init_size * 2, partition="test", path=data_path
    )
    train_loader = DataLoader(dataset=train_loader, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(dataset=test_loader, batch_size=test_size, shuffle=False)
