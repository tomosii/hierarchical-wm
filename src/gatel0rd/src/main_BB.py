...
from utils.gating_analysis import (
    calc_gate_rate,
    calc_gate_dims_used,
    calc_gate_open_times,
)
...
if __name__ == "__main__":
    ...
    wandb.init(project="gatel0rd", config=params)
    ...
    if network_name == "GateL0RD":
        net = GateL0RD.GateL0RDModel(
            input_dim=input_dim,
            output_dim=output_dim,
            latent_dim=params.latent_dim,
            feature_dim=params.feature_dim,
            num_layers=params.layer_num,
            ...
            gate_noise_level=params.gate_noise_level,
    ...
    (
        ...
        val_dataloaders,
        test_dataloaders,
    ) = bb_dataloader.get_billiard_ball_dataloaders(
        ...
        if epoch % t_validation == 0:
            ...
            with torch.no_grad():
                ...
                for (
                    ...
                    y, z, gate_reg, out_hidden, deltas = net.forward_n_step(
                        obs_batch=inp,
            ...
            wandb.log(
                {
                    "validation_MSE": val_MSE_over_t[validations],
                    "test_MSE": test_MSE_over_t[validations],
                    "test_gate_rate": test_gate_rate[validations],
                    "test_gate_dims": test_gate_dims[validations],
                    "test_gate_times": test_gate_times[validations],
                },
                step=epoch,
            )
            ...
            dir_name_checkpoint = os.path.join(
                target_dir, "checkpoint_v" + str(validations)
            )
            # Save a checkpoint
            torch.save(
                {
            ...
            for (
                ...
                if not teacherforcing:
                    if ss_slope == 0:
                        ...
                        ss_epsilon = exponential_ss_prob(
                            epoch=epoch, slope=ss_slope, min_value=0.0
                        )
